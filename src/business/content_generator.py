...
FALLBACK_POSTS = {
    "morning": """\
AI副業を始めて2週間経った、正直な話をする

・収益: まだゼロ
・作ったもの: Threads自動投稿の仕組み
・かかった費用: ほぼ0円（無料枠だけ）

うまくいってないけど、仕組みは動いてる。
動いてる仕組みが一個あると、次に進める気がする。

同じように試してる人いる？""",

    "noon": """\
正直に言うと、Threads自動投稿を作っても最初は全然うまくいかなかった

失敗したこと:
・重複投稿が発生した
・トークンの有効期限が切れた
・GitHubのSecretの設定を間違えた

全部Claude Codeに聞きながら解決した。
コードが書けなくても、エラー文を貼れば進める。

やり方間違えてたら遠慮なく突っ込んでください。""",

    "evening": """\
【手順メモ】GitHub ActionsでThreads自動投稿を作った方法

1. Meta DeveloperでThreads APIアプリを作成
2. アクセストークンを取得（有効期限60日）
3. Gemini APIで投稿文を自動生成
4. GitHub Actionsで1日2回自動投稿

費用: 0円
コード知識: ほぼ不要（Claude Codeに全部聞いた）
